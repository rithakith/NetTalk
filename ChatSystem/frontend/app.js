  ...
  try {
    ...
    ws.onerror = (error) => {
      ...
      addMessage(
        "SYSTEM",
        "Error",
        "Failed to connect to WebSocket bridge server on port 8889"
      );
      addMessage(
        "SYSTEM",
        "Info",
        "Please ensure: 1) ChatServer is running on port 8888, 2) WebSocketServer is running on port 8889"
      );
      addMessage(
        "SYSTEM",
        "Info",
        'Run: java -cp ".:lib/*" com.chatapp.server.WebSocketServer'
      );
      alert(
...
function handleMessage(message) {
  ...
  switch (message.type) {
    ...
    case "QUIZ_QUESTION":
      console.log(
        "[WebSocket] Received QUIZ_QUESTION message:",
        message.content
      );
...
function disconnect() {
  ...
  // Logout user and return to auth page
...
function addMessage(
  ...
  if (type === "CHAT" && sender === username && messageId) {
    console.log(
      "Adding status indicator for message:",
      messageId,
      "status:",
      status
    );
...
function updateUserList(users) {
  ...
  users.forEach((user) => {
    ...
    if (user === username) {
      ...
      li.className =
        currentPrivateChat === user ? "user-item-active" : "user-item";
 ...
 */
function addPrivateMessageToHistory(
  targetUser,
  message,
  sender,
  messageId = null
  ...
  const msgObj = {
    ...
    read:
      sender === username ||
      (chatView === "private" && currentPrivateChat === targetUser),
...
function updateStatusIcon(statusElement, status) {
  ...
  switch (status) {
    ...
    case "SEEN":
      ...
      statusElement.innerHTML =
        ' <span class="status-icon status-seen">✓✓</span>';
...
function sendPrivateMessageDelivered(messageId, fromUser) {
  if (ws && ws.readyState === WebSocket.OPEN) {
    try {
      ...
      console.log(
        `Sent delivery confirmation for private message ${messageId}`
      );
    } catch (error) {
      console.error(
        "Failed to send private message delivery confirmation:",
        error
      );
    }
  }
  ...
  try {
    ...
    if (messages.length > 0) {
      ...
      separatorDiv.innerHTML =
        '<div class="message-content"><em>--- Previous Messages ---</em></div>';
      ...
      separator2Div.innerHTML =
        '<div class="message-content"><em>--- New Session ---</em></div>';
...
window.clearChatHistory = function () {
  if (
    confirm(
      "Are you sure you want to clear all chat history from local storage?"
    )
  ) {
...
function createQuizListItem(quiz, status) {
  ...
  item.innerHTML = `
        ...
        <div class="quiz-list-item-actions">
            <button class="btn-quiz-action btn-quiz-join" onclick="joinQuiz('${
              quiz.id
            }')">
                Join
            </button>
            <button class="btn-quiz-action btn-quiz-view" onclick="viewQuizDetails('${
              quiz.id
            }')">
                Details
            </button>
...
function parseQuizList(content) {
  ...
  for (let line of lines) {
    const match = line.match(
      /- (.+) \(ID: ([a-zA-Z0-9]+)\) - Admin: (.+) - State: ([A-Z]+) - Questions: (\d+)/
    );
    if (match) {
  ...
  updateAvailableQuizzes();
  console.log(
    `Loaded ${serverQuizzes.size} quizzes from server, total: ${activeQuizzes.size}`
  );
}
...
function handleQuizMessage(message) {
  ...
  if (content.includes("Quiz") && content.includes("created successfully")) {
    const match = content.match(
      /Quiz '(.+)' created successfully! Quiz ID: ([a-zA-Z0-9]+)/
    );
    if (match) {
      const quizName = match[1];
      const quizId = match[2];
...
function handleQuizDeleted(message) {
  ...
  if (match) {
    ...
    console.log(
      "📊 Active quizzes before deletion:",
      Array.from(activeQuizzes.keys())
    );
    ...
    console.log(
      "📊 Active quizzes after deletion:",
      Array.from(activeQuizzes.keys())
    );
  } else {
    console.error(
      "❌ Failed to extract quiz ID from deletion message:",
      content
    );
  }
}
...
function parseAndDisplayQuiz(content) {
  ...
  console.log(
    "[Quiz] Content length:",
    content ? content.length : "null/undefined"
  );
  ...
  if (match) {
    ...
    if (!joinedQuizId) {
      ...
      for (const [id, quiz] of activeQuizzes.entries()) {
        if (quiz.name === quizName) {
          joinedQuizId = id;
          console.log(
            "[Quiz] Found quiz ID from active quizzes:",
            joinedQuizId
          );
          break;
        }
      ...
      if (!joinedQuizId) {
        console.error(
          "[Quiz] ERROR: Could not find quiz ID for quiz:",
          quizName
        );
      }
    }
    ...
    if (altMatch) {
      console.warn(
        "[Quiz] Found quiz name using alternative regex:",
        altMatch[1]
      );
    }
  }
...
function displayQuizInterface(quizName) {
  ...
  console.log(
    "[Quiz] messageArea display:",
    messageArea ? messageArea.style.display : "N/A"
  );
  ...
  const totalQs =
    currentQuiz && currentQuiz.totalQuestions > 0
  ...
  quizContainer.innerHTML = `
        <div class="quiz-header-display">
            ...
            <div class="quiz-title-display">
                <h2>${quizName}</h2>
                <p class="quiz-subtitle">Select your answers and submit when ready • ${totalQs} question${
    totalQs !== 1 && totalQs !== "?" ? "s" : ""
  }</p>
            </div>
        </div>
  ...
  console.log(
    "[Quiz] messageArea children count:",
    messageArea.children.length
  );
...
function updateQuizInterface() {
  ...
  console.log(
    "[Quiz] Container display:",
    container ? getComputedStyle(container).display : "N/A"
  );
  console.log(
    "[Quiz] Container dimensions:",
  ...
  quizQuestions.forEach((question, index) => {
    ...
    question.options.forEach((option) => {
      ...
      const isSelected =
        quizAnswers.get(question.number) === option.letter.charCodeAt(0) - 65; // A=0, B=1, C=2, D=3
      optionsHTML += `
                <label class="quiz-option ${isSelected ? "selected" : ""}">
                    <input type="radio" 
                           ...
                           value="${option.letter}"
                           ${isSelected ? "checked" : ""}
                           onchange="selectQuizOption(${question.number}, '${
        option.letter
      }')">
    ...
    console.log(
      "[Quiz] Options HTML for question",
      question.number,
      ":",
      optionsHTML.length,
      "chars"
    ...
    questionDiv.innerHTML = `
            <div class="question-header">
                ...
                <span class="question-status ${
                  quizAnswers.has(question.number) ? "answered" : "unanswered"
                }">
                    ${
                      quizAnswers.has(question.number)
                        ? "✓ Answered"
                        : "⚪ Not Answered"
                    }
                </span>
    ...
    console.log(
      "[Quiz] Question div appended, height:",
  ...
  const messageArea = document.getElementById("messageArea");
  console.log(
    "[Quiz] Quiz container height:",
    quizContainer ? quizContainer.offsetHeight : "not found"
  );
  console.log(
    "[Quiz] Message area height:",
    messageArea ? messageArea.offsetHeight : "not found"
  );
  console.log(
    "[Quiz] Message area display:",
    messageArea ? getComputedStyle(messageArea).display : "N/A"
  );
...
function updateSubmitButton() {
  ...
  console.log(
    "[Quiz] Submit button updated - Answered:",
    quizAnswers.size,
    "Expected:",
    expectedQuestions,
    "Enabled:",
...
function submitQuiz() {
  ...
  quizQuestions.forEach((question) => {
    ...
    console.log(
      `Submitting answer for question ${question.number}: ${answerCmd}`
    );
  ...
  console.log(
    `✅ Quiz submitted: ${quizAnswers.size} answers for quiz ${currentQuiz.id}`
  );
}
...
function cancelQuiz() {
  if (
    confirm(
      "Are you sure you want to cancel this quiz? Your answers will be lost."
    )
  ) {
    currentQuiz = null;
    quizAnswers.clear();
...
window.checkQuizDisplay = function () {
  ...
  console.log(
    "  - dimensions:",
    messageArea
      ? `${messageArea.offsetWidth}x${messageArea.offsetHeight}`
      : "N/A"
  );
  console.log(
    "  - children:",
    messageArea ? messageArea.children.length : "N/A"
  );

  console.log("Quiz container:", quizContainer);
  console.log(
    "  - display:",
    quizContainer ? getComputedStyle(quizContainer).display : "N/A"
  );
  console.log(
    "  - dimensions:",
    quizContainer
      ? `${quizContainer.offsetWidth}x${quizContainer.offsetHeight}`
      : "N/A"
  );
  console.log(
    "  - visibility:",
    quizContainer ? getComputedStyle(quizContainer).visibility : "N/A"
  );
  console.log(
    "  - children:",
    quizContainer ? quizContainer.children.length : "N/A"
  );
  console.log(
    "  - className:",
    quizContainer ? quizContainer.className : "N/A"
  );

  console.log("Questions container:", questionsContainer);
  console.log(
    "  - display:",
    questionsContainer ? getComputedStyle(questionsContainer).display : "N/A"
  );
  console.log(
    "  - dimensions:",
    questionsContainer
      ? `${questionsContainer.offsetWidth}x${questionsContainer.offsetHeight}`
      : "N/A"
  );
  console.log(
    "  - children:",
    questionsContainer ? questionsContainer.children.length : "N/A"
  );
  ...
  if (questionsContainer && questionsContainer.children.length > 0) {
    ...
    console.log(
      "  - display:",
      getComputedStyle(questionsContainer.children[0]).display
    );
    console.log(
      "  - dimensions:",
      `${questionsContainer.children[0].offsetWidth}x${questionsContainer.children[0].offsetHeight}`
    );
  }
...
window.testQuizDisplay = function (quizName) {
  ...
  console.log(
    "Calling displayQuizInterface with name:",
    quizName || "TestQuiz"
  );
