            ...
            clientConn.readerThread = new Thread(() -> {
                try {
                    while (!Thread.interrupted() && conn.isOpen()) {
                        ...
                        // Convert ALL message types to JSON for frontend
                        ...
                        System.out.println("[WebSocket] Forwarding to frontend: " + msg.getType() + " - " + msg.getContent());
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        try {
            ...
            switch (type) {
                ...
                case "FILE_SEND":
                    msg = new Message(Message.MessageType.FILE_SEND,
                            jsonMsg.get("sender").getAsString(),
                            jsonMsg.get("content").getAsString());
                    msg.setFileName(jsonMsg.get("fileName").getAsString());
                    msg.setFileSize(jsonMsg.get("fileSize").getAsLong());
                    break;
                ...
                case "REGISTER":
                    msg = new Message(Message.MessageType.REGISTER,
                            jsonMsg.get("sender").getAsString(),
                            jsonMsg.get("content").getAsString());
                    break;

                case "LOGIN":
                    msg = new Message(Message.MessageType.LOGIN,
                            jsonMsg.get("sender").getAsString(),
                            jsonMsg.get("content").getAsString());
                    break;

                default:
                    System.out.println("[WebSocket] Unknown message type: " + type);
            }

            if (msg != null && clientConn.tcpOut != null) {
                System.out.println("[WebSocket] Forwarding to TCP: " + msg.getType() + " - " + msg.getSender());
